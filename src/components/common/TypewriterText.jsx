...
export default function TypewriterText({
  ...
  const currentPhrase = phrases[currentPhraseIndex] || '';
  const isComplete = !isDeleting && displayedText.length === currentPhrase.length && displayedText.length > 0;
  
  // Determine if cursor should be visible
  let shouldShowCursor = true;
  if (hideCursorOnComplete && isComplete) {
    shouldShowCursor = false;
  } else if (hideCursorOnDelete && isDeleting) {
    shouldShowCursor = false;
  }
      ...
      {shouldShowCursor && (
        <span
          ...
          style={{
            ...
            WebkitTextFillColor: 'var(--accent-rose, #ff4d7a)',
